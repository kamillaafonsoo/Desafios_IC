...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        # 1 dimensão para o raio e 63 dimensões para o RFF
        num_rff = self.DIM_MAX - 1 

        d2 = torch.cdist(Xs[:300], Xs[:300]) ** 2
        mask = d2 > 1e-4
        sigma = math.sqrt(d2[mask].median().item() / 2.0) if mask.any() else 1.0

        g = torch.Generator().manual_seed(42)
        self.W = torch.randn(X.shape[1], num_rff, generator=g, device=X.device, dtype=X.dtype) / sigma
        self.b = (torch.rand(num_rff, generator=g, device=X.device, dtype=X.dtype) * 2 * math.pi)

    def phi(self, X: torch.Tensor) -> torch.Tensor:
        Xs = (X - self.mu) / self.sd

        # 1. Distância ao quadrado (1 dimensão)
        feat_raio = (Xs ** 2).sum(dim=1, keepdim=True)

        # 2. RFF com precisão dupla (float64) para garantir o determinismo
        Xs_d = Xs.to(torch.float64)
        W_d = self.W.to(torch.float64)
        b_d = self.b.to(torch.float64)

        proj = Xs_d @ W_d + b_d
        
        # Volta para o tipo original após a multiplicação
        feat_rff = math.sqrt(2.0 / self.W.shape[1]) * torch.cos(proj).to(X.dtype)

        return torch.cat([feat_raio, feat_rff], dim=1)
